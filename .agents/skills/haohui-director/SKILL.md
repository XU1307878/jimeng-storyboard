...
11. Carry the outgoing body pose, gaze, prop contact, light state and sound cue into the next shot's entry state. For movement, preserve `start → route → interaction point → endpoint`, and reserve enough frame space for the next meaningful action without revealing delayed entrants early. At dialogue reverse angles, pursuits, entrances/exits or continuous-action cuts, also record the relationship axis, camera-safe side, outgoing/incoming screen direction and matching eyelines; a side change is valid only after visible crossing, a neutral-axis shot or spatial re-establishment.
