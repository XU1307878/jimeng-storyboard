...
17a. Optimize the model-execution payload in semantic priority order: `current-shot references and roles → subject + visible action → camera observation → timed beat changes → end state/continuity → essential light/style → risk-specific exclusions`. Do not simulate weight with repeated keywords or invented numeric syntax.
