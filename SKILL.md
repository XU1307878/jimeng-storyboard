...
9d. Assign the endpoint one primary purpose: `无缝续拍`, `下一镜切换`, `过桥`, `尾部修复` or `重新定场`. Let that purpose determine whether the ending settles, remains in motion, preserves a transition cue or re-establishes geography.
...
15a. When the user explicitly selects a generated take, use its actual observable end state as the next clip's entry truth and label whether it was directly observed or user-reported. An unselected candidate never overrides planned continuity.
